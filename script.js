...
checkout.addEventListener("click", function(){
    ...
    // enviar API
    const cartItemsContainers = cart.map((item)  => {
     return(
            `${item.name} quantidade: (${item.quantidade}) preço: ${item.price} |` 
        )

    }).join("")
    

   const message = encodeURIComponent(cartItemsContainers)
   
   const phone = "8194860445"
